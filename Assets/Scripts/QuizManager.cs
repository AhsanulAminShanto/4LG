...
// Using TextMeshPro for better text rendering (recommended)
...
public class QuizManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject wrongAns;
    [SerializeField] private GameObject settingsAns;
    [SerializeField] private GameObject hintsAns;
    ...
    public void SelectedOption(WordData value)
    {
        ...
        if (currentAnswerIndex == answerWord.Length)
        {
            ...
            else
            {
                ...
                StartCoroutine(HideWrongAnswer());
            }
        }
    }
    private IEnumerator HideWrongAnswer()
    {
    ...
    {
        settingsAns.SetActive(true);
        StartCoroutine(HideSettings()); // Start coroutine correctly
    }

    public void ShowHints()
    {
        hintsAns.SetActive(true);
        StartCoroutine(HideHints()); // Start coroutine correctly
    }
    private IEnumerator HideSettings()
    {
        yield return new WaitForSeconds(1f); // Wait for 1 second
        settingsAns.SetActive(false);
    }
    private IEnumerator HideHints()
    {
        yield return new WaitForSeconds(1f); // Wait for 1 second
        hintsAns.SetActive(false);
    }
